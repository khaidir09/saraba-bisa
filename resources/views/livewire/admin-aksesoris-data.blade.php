<div>
    ...
    <div class="sm:flex sm:justify-between sm:items-center mb-3">
        ...
        <div class="mb-4 sm:mb-0">
            <h1 class="text-2xl md:text-3xl text-slate-800 font-bold">Aksesori ✨</h1>
        </div>
        ...
        <div class="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
            ...
            <div x-data="{ modalOpen: false }">
                <button class="btn bg-indigo-500 hover:bg-indigo-600 text-white" @click.prevent="modalOpen = true" aria-controls="tambah-modal">
                    ...
                    <span class="hidden xs:block ml-2">Tambah Aksesori</span>
                </button>
                <!-- Modal backdrop -->
                <div
                    ...
                    <div class="bg-white rounded shadow-lg overflow-auto max-w-lg w-full max-h-full" @click.outside="modalOpen = false" @keydown.escape.window="modalOpen = false">
                        ...
                        <div class="px-5 py-3 border-b border-slate-200">
                            <div class="flex justify-between items-center">
                                <div class="font-semibold text-slate-800">Tambah Aksesori</div>
                        ...
                        <!-- Modal content -->
                        <form action="{{ route('admin-data-aksesoris.store') }}" method="post">
                            @csrf
                            <div class="px-5 py-4">
                                <div class="space-y-3">
                                    <div>
                                        <label class="block text-sm font-medium mb-1" for="name">Nama Aksesori <span class="text-rose-500">*</span></label>
                            ...
                            <div class="px-5 py-4 border-t border-slate-200">
                                <div class="flex flex-wrap justify-end space-x-2">
                                    <a href="{{ route('admin-data-aksesoris.index') }}" class="btn-sm border-slate-200 hover:border-slate-300 text-slate-600">
                                        Batal
                                    </a>
    ...
    <div class="sm:flex sm:justify-between sm:items-center mb-5">
        ...
        <div class="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
            <!-- Start Export Excel -->
            <a href="{{ route('admin-aksesori-export') }}" class="hidden lg:block">
            ...
            <div x-data="{ modalOpen: false }" class="hidden lg:block">
                    ...
                    >
                        <div class="bg-white rounded shadow-lg overflow-auto max-w-xl w-full max-h-full" @click.outside="modalOpen = false" @keydown.escape.window="modalOpen = false">
                            <form action="{{ route('admin-impor-aksesori') }}" method="post" enctype="multipart/form-data">
                            @csrf
                                <!-- Modal header -->
                                <div class="px-5 py-3 border-b border-slate-200">
                                    <div class="flex justify-between items-center">
                                        <div class="font-semibold text-slate-800">Impor Data Aksesori</div>
                                        <button class="text-slate-400 hover:text-slate-500" @click="modalOpen = false">
                                            <div class="sr-only">Close</div>
                                            <svg class="w-4 h-4 fill-current">
                                                <path d="M7.95 6.536l4.242-4.243a1 1 0 111.415 1.414L9.364 7.95l4.243 4.242a1 1 0 11-1.415 1.415L7.95 9.364l-4.243 4.243a1 1 0 01-1.414-1.415L6.536 7.95 2.293 3.707a1 1 0 011.414-1.414L7.95 6.536z" />
    ...
    <div class="bg-white shadow-lg rounded-sm border border-slate-200 mt-5 mb-8">
        <header class="px-5 py-4">
            <h2 class="font-semibold text-slate-800">Semua Aksesoris <span class="text-slate-400 font-medium">{{ $accessories_count }}</span></h2>
        </header>
        <!-- Table -->
            ...
            <table class="table-auto w-full">
                ...
                <thead class="text-xs font-semibold uppercase text-slate-500 bg-slate-50 border-t border-b border-slate-200">
                    <tr>
                        ...
                        <th class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                            <div class="font-semibold text-left">Nama Aksesoris</div>
                        </th>
                        <th class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                            <div class="font-semibold text-left">Jumlah Stok</div>
                        </th>
                        <th class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                            <div class="font-semibold text-left">Modal Aksesoris</div>
                        </th>
                        <th class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                ...
                <tbody class="text-sm divide-y divide-slate-200">
                    ...
                    @php
                        $i = 1
                    @endphp
                    @foreach($accessories as $aksesoris)                  
                        <tr>
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                <div class="font-medium">{{ $i++ }}</div>
                            </td>
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                <div class="font-medium">{{ $aksesoris->name }}</div>
                            </td>
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                <div class="font-medium">{{ $aksesoris->stok }}</div>
                            </td>
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                <div class="font-medium">Rp. {{ number_format($aksesoris->modal) }}</div>
                            </td>
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                <div class="font-medium">Rp. {{ number_format($aksesoris->harga_toko) }}</div>
                            </td>
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                <div class="font-medium">Rp. {{ number_format($aksesoris->harga_pelanggan) }}</div>
                            </td>
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                <div class="font-medium">{{ $aksesoris->supplier }}</div>
                            </td>
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap w-px">
                                <div class="space-x-1 flex">
                                    <a href="{{ route('admin-data-aksesoris.edit', $aksesoris->id) }}">
                                                ...
                                                <div class="p-5 flex space-x-4">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div class="flex flex-wrap justify-end space-x-2">
                                                            ...
                                                            <form action="{{ route('admin-data-aksesoris.destroy', $aksesoris->id) }}" method="post">
                                                                @method('delete')
                                                                @csrf
