<div>
                    ...
                    @foreach($processes as $process)
                        ...
                        <tr>
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                <button wire:click="addToInvoice({{ $process->id }})">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-file-plus" width="20" height="20" viewBox="0 0 24 24" stroke-width="1.5" stroke="#009988" fill="none" stroke-linecap="round" stroke-linejoin="round">
                            ...
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                <div class="flex space-x-1">
                                    ...
                                    >
                                        ...
                                        @else
                                            <a href="https://wa.me/{{ $nomorwa }}/?text=*Notifikasi%20|%20{{ $toko->nama_toko }}*%20Barang%20Servis%20{{ $process->type->name }}%20{{ $process->brand->name }}%20{{ $process->modelserie->name }}%20telah%20diterima%20oleh%20*{{ $process->penerima }}*%20dengan%20No.%20Servis%20*{{ $process->nomor_servis }}*%20pada%20tanggal%20{{ \Carbon\Carbon::parse($process->created_at)->translatedFormat('d F Y h:i') }}%20WIB.%20Untuk%20Cek%20Status%20(Tracking)%20Servis%20barang%20Anda,%20silahkan%20buka%20Link%20berikut%20ini%20{{ $toko->link_toko }}/tracking.%20Terima%20Kasih." target="__blank">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-file-invoice" width="20" height="20" viewBox="0 0 24 24" stroke-width="1.5" stroke="#00abfb" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                                <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                                                <path d="M14 3v4a1 1 0 0 0 1 1h4" />
                                                <path d="M17 21h-10a2 2 0 0 1 -2 -2v-14a2 2 0 0 1 2 -2h7l5 5v11a2 2 0 0 1 -2 2z" />
    ...
    <div class="sm:flex sm:justify-between sm:items-center mb-3">
        ...
        <!-- Left: Title -->
        <div class="mb-4 sm:mb-0">
            <h1 class="text-2xl md:text-3xl text-slate-800 font-bold">Cetak Nota Gabungan ✨</h1>
        </div>
    ...
    <div class="bg-white shadow-lg rounded-sm border border-slate-200 mt-5 mb-8">
        <header class="px-5 py-4">
            <h2 class="font-semibold text-slate-800">Gabungan Data</h2>
        </header>
        <!-- Table -->
        <div class="overflow-x-auto">
            <table class="table-auto w-full">
                ...
                <tbody class="text-sm divide-y divide-slate-200">
                    ...
                    @foreach($invoiceItems as $item)
                        <tr>
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-file-minus" width="20" height="20" viewBox="0 0 24 24" stroke-width="1.5" stroke="#ff2825" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                                <path d="M14 3v4a1 1 0 0 0 1 1h4" />
                                <path d="M17 21h-10a2 2 0 0 1 -2 -2v-14a2 2 0 0 1 2 -2h7l5 5v11a2 2 0 0 1 -2 2z" />
                                <path d="M9 14l6 0" />
                            ...
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                <div class="font-medium">{{ $item->type->name }} {{ $item->brand->name }} {{ $item->modelserie->name }}</div>
                            </td>
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                <div class="font-medium capitalize">{{ $item->kerusakan }}</div>
                            </td>
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                <div class="font-medium">Rp. {{ number_format($item->uang_muka) }}</div>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
